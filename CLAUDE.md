...
- `useKeyboardShortcuts` — attaches a single `keydown` listener on `window` and maps all keyboard shortcuts. Skips events when the target is a text input or when the shortcut overlay is open. Receives `activeTab` and `toggleValidatorOption` to support tab-scoped shortcuts: the validator accept-option keys (`⌥R`, `⌥[`, `⌥-`) only fire when `activeTab === "validator"`.
